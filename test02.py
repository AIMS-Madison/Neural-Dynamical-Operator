...
# setup
epochs = 3000 # overwrite previous epochs
loss_training_history = []
eki_freq = 300 # ratio of epochs (sgd / eki)
num_EKI = 0
        ...
        for m in model_sgd.children():
            if not isinstance(m, type(model_sgd.conv0)):
                with torch.no_grad():
                    for param in m.parameters():
                        param.data = param.data.real
        optimizer = torch.optim.Adam(model_sgd.parameters(), lr=1e-3) # Each SGD round will have a new optimizer

torch.save(theta_bar, "/home/cc/CodeProjects/NeuralDynamicalOperator/SGD_EKI/test02/theta_eki.pt")
with open("/home/cc/CodeProjects/NeuralDynamicalOperator/SGD_EKI/test02/loss_training_history.npy", "wb") as f:
    np.save(f, loss_training_history)
with open("/home/cc/CodeProjects/NeuralDynamicalOperator/SGD_EKI/test02/error_state_history_eki.npy", "wb") as f:
    np.save(f, error_state_history_eki)
with open("/home/cc/CodeProjects/NeuralDynamicalOperator/SGD_EKI/test02/error_kurtosis_history_eki.npy", "wb") as f:
    np.save(f, error_kurtosis_history_eki)
np.save('/home/cc/CodeProjects/NeuralDynamicalOperator/SGD_EKI/test02/theta_bar_history.npy', np.array(theta_bar_history_eki, dtype=object), allow_pickle=True)


# eki_theta = torch.load("/home/cc/CodeProjects/NeuralDynamicalOperator/SGD_EKI/test02/theta_eki.pt")
